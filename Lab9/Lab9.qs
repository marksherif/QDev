...
namespace Lab9 {
    ...
    operation Exercise1_ModExp (
        ...
        //     for j in 0 .. (1 <<< i) - 1 {
        //         let angle = 2.0 * PI() * IntAsDouble(a) * j / (1 <<< i) % 1.0;
        //         Controlled ExpModI(angle, input[i], output);
        //     }
        // }
    ...
    operation Exercise2_FindApproxPeriod (
        ...
        use input = Qubit[numberToFactor ^ 2];
        use output = Qubit[numberToFactor ^ 2];
    ...
    function Exercise3_FindPeriodCandidate (
        ...
        mutable currentNumerator = numerator;
        mutable currentDenominator = denominator;
        mutable previousNumerator = 1;
        mutable previousDenominator = 0;

        while (currentDenominator < denominatorThreshold) {
            let integerPart = currentNumerator / currentDenominator;
            let remainder = currentNumerator % currentDenominator;

            if (remainder == 0) {
                return (currentNumerator, currentDenominator);
            }

            let nextNumerator = previousNumerator + integerPart * currentNumerator;
            let nextDenominator = previousDenominator + integerPart * currentDenominator;

            set previousNumerator = currentNumerator;
            set previousDenominator = currentDenominator;
            set currentNumerator = nextNumerator;
            set currentDenominator = nextDenominator;
        }

        return (previousNumerator, previousDenominator);
    }
    ...
    operation Exercise4_FindPeriod (numberToFactor : Int, guess : Int) : Int
    {
        ...
        let gcd = GreatestCommonDivisorI(r, N);
        if gcd != 1 {
            return gcd;
        }
