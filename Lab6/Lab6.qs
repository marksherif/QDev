...
namespace Lab6 {
    ...
    operation Exercise1 (
        ...
        use (input_register, outout_register) = (Qubit[Length(input)], Qubit[Length(input)]);

        // Prepare the state so its like the input string
        for i in 0 .. Length(input) - 1 {
            if (input[i] == true) {
                X(input_register[i]);
            }
        ...
        op(input_register, outout_register);

        mutable results = [false, size = Length(input)];
        for i in 0 .. Length(input) - 1 {
            if (M(outout_register[i]) == One){
                set results w/= i <- true;
            }
            else{
                set results w/= i <- false;
            }
        }

        ResetAll(input_register);
        ResetAll(outout_register);
    ...
    operation Exercise2 (
        ...
        use (input_register, outout_register) = (Qubit[inputSize], Qubit[inputSize]);

        ApplyToEach(H, input_register);
        op(input_register, outout_register);
        ApplyToEach(H, input_register);

        mutable results = [false, size = inputSize];
        for i in 0 .. inputSize - 1 {
            if (M(input_register[i]) == One){
                set results w/= i <- true;
            }
            else{
                set results w/= i <- false;
